...
// Display an indicator to identify if the product belongs at a customer group or not.
        ...
        $this->products[$index] = ['qty' => $products[$i]['quantity'],
                                    ...
                                    'final_price' => $products[$i]['price'] + $CLICSHOPPING_ShoppingCart->attributes_price($products[$i]['id']),
...
//Display only in shoppingCart
            ...
            $this->products[$index] = ['qty' => $products[$i]['quantity'],
                                       ...
                                       'final_price' => $QordersCustomersPrice->valueDecimal('customers_group_price') + $CLICSHOPPING_ShoppingCart->attributes_price($products[$i]['id']),
                                       'weight' => $products[$i]['weight'],
                                       'id' => $products[$i]['id']
                                      ];
...
//temporaire - CMCIC / Monéris / Desjardins
        ...
        $this->mail->oscMail($this->customer['firstname'] . ' ' . $this->customer['lastname'], $this->customer['email_address'], CLICSHOPPING::getDef('email_text_subject', ['store_name' => STORE_NAME]), $email_order, STORE_NAME, STORE_OWNER_EMAIL_ADDRESS);
...
// SEND_EXTRA_ORDER_EMAILS_TO does'nt work like this, test<test@test.com>, just with test@test.com
        if (SEND_EXTRA_ORDER_EMAILS_TO != '') {
          ...
          foreach($text as $email){
            $this->mail->oscMail('', $email, $email_text_subject, $email_order, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
          }
        }
      }
    }
        ...
        if ( $Qproducts->fetch() !== false ) {
          ...
          while ( $Qproducts->fetch() ) {
            ...
            if (($stock_left < 1) && (STOCK_ALLOW_CHECKOUT == 'false') && (STOCK_CHECK == 'true')) {
              ...
              $this->mail->oscMail(STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, $email_text_subject_stock, $email_product_exhausted_stock, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
            }
          } // end stock alert
        }  // end while
      }
...
// alert email if stock product alert < warning stock
              if (($stock_products_quantity_alert <= $warning_stock) && ($stock_products_quantity_alert != '0')) {
                ...
                $this->mail->oscMail(STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, $email_text_subject_stock, $reorder_stock_email, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
              }
