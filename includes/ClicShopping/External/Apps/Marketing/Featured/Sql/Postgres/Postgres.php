...
namespace ClicShopping\Apps\Marketing\Featured\Sql\Postgres;
...
class Postgres
{
  ...
  private static function installDb(): void
  {
    ...
    $Qcheck = $CLICSHOPPING_Db->query("SELECT to_regclass(':table_products_featured')");

    if ($Qcheck->fetch() === false) {
      $sql = <<<EOD
    CREATE TABLE :table_products_featured (
      products_featured_id serial PRIMARY KEY,
      products_id int NOT NULL default(0),
      products_featured_date_added timestamp,
      products_featured_last_modified timestamp,
    ...
    CREATE INDEX idx_products_featured_id ON :table_products_featured (products_id);
EOD;
