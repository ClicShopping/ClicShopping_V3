<?php
/**
 *
 * @copyright 2008 - https://www.clicshopping.org
 * @Brand : ClicShoppingAI(TM) at Inpi all right Reserved
 * @Licence GPL 2 & MIT
 * @Info : https://www.clicshopping.org/forum/trademark/
 *
 */

namespace ClicShopping\Apps\Tools\Upgrade\Sql\Postgres;

use ClicShopping\OM\Cache;
use ClicShopping\OM\Registry;

class Postgres
{
  public function execute()
  {
    $CLICSHOPPING_Upgrade = Registry::get('Upgrade');
    $CLICSHOPPING_Upgrade->loadDefinitions('Sites/ClicShoppingAdmin/install');

    self::installMenuAdministration();
    self::installDb();
  }

  /**
   * @return void
   */
  private static function installMenuAdministration(): void
  {
    $CLICSHOPPING_Db = Registry::get('Db');
    $CLICSHOPPING_Upgrade = Registry::get('Upgrade');
    $CLICSHOPPING_Language = Registry::get('Language');

    $Qcheck = $CLICSHOPPING_Db->get('administrator_menu', 'app_code', ['app_code' => 'app_tools_upgrade']);

    if ($Qcheck->fetch() === false) {
      $sql_data_array = [
        'sort_order' => 1,
        'link' => 'index.php?A&Tools\Upgrade&Upgrade',
        'image' => 'menu.png',
        'b2b_menu' => 0,
        'access' => 1,
        'app_code' => 'app_tools_upgrade'
      ];

      $insert_sql_data = ['parent_id' => 163];

      $sql_data_array = array_merge($sql_data_array, $insert_sql_data);
      $CLICSHOPPING_Db->save('administrator_menu', $sql_data_array);

      $id = $CLICSHOPPING_Db->lastInsertId();
      $languages = $CLICSHOPPING_Language->getLanguages();

      for ($i = 0, $n = \count($languages); $i < $n; $i++) {
        $language_id = $languages[$i]['id'];
        $sql_data_array = ['label' => $CLICSHOPPING_Upgrade->getDef('title_menu')];

        $insert_sql_data = [
          'id' => (int)$id,
          'language_id' => (int)$language_id
        ];

        $sql_data_array = array_merge($sql_data_array, $insert_sql_data);

        $CLICSHOPPING_Db->save('administrator_menu_description', $sql_data_array);
      }

      Cache::clear('menu-administrator');
    }
  }

  /**
   * @return void
   */
  private static function installDb()
  {
    $CLICSHOPPING_Db = Registry::get('Db');

    $Qcheck = $CLICSHOPPING_Db->query("SELECT to_regclass(':table_marketplace_categories')");

    if ($Qcheck->fetch() === false) {
      $sql = <<<EOD
    CREATE TABLE :table_marketplace_categories (
      id serial PRIMARY KEY,
      categories_id int NOT NULL,
      parent_id int DEFAULT 0 NOT NULL,
      categories_name text,
      url text,
      date_added date,
      date_modified date,
      sort_order int DEFAULT 0
    );

    CREATE INDEX idx_parent_id ON :table_marketplace_categories (parent_id);

    CREATE TABLE :table_marketplace_files (
      id serial PRIMARY KEY,
      file_id int NOT NULL,
      file_categories_id int NOT NULL,
      file_name varchar(255),
      file_url text,
      file_description text,
      file_author varchar(255) NOT NULL,
      file_photo_url text NOT NULL,
      file_profil_url text NOT NULL,
      date_added date,
      date_modified date
    );

    CREATE INDEX file_id ON :table_marketplace_files (file_id);

    CREATE TABLE :table_marketplace_file_informations (
      id serial PRIMARY KEY,
      file_id int NOT NULL,
      file_name varchar(255),
      date_created date,
      date_updated date,
      file_version varchar(255),
      file_downloads int,
      file_rating int,
      file_prices decimal(15,4),
      file_date_added date,
      file_url_screenshot text,
      file_url_download text,
      is_installed smallint NOT NULL DEFAULT 0
    );

    CREATE INDEX index_file_id ON :table_marketplace_file_informations (file_id);
EOD;
      $CLICSHOPPING_Db->exec($sql);
    }
  }
}
