...
namespace ClicShopping\Apps\Catalog\Categories\Sql\Postgres;
 ...
 */
  ...
  {
    $CLICSHOPPING_Db = Registry::get('Db');
    $Qcheck = $CLICSHOPPING_Db->query("SELECT to_regclass(':table_categories')");
...
CREATE TABLE :table_categories (
  ...
  virtual_categories smallint DEFAULT 0 NOT NULL,
  status smallint DEFAULT 0 NOT NULL,
  customers_group_id int DEFAULT 99 NOT NULL,
  CONSTRAINT idx_categories_parent_id FOREIGN KEY (parent_id) REFERENCES :table_categories(categories_id)
) ;
EOD;
      $CLICSHOPPING_Db->exec($sql);
    }

    $Qcheck = $CLICSHOPPING_Db->query("SELECT to_regclass(':table_categories_description')");
...
CREATE TABLE :table_categories_description (
  ...
  CONSTRAINT idx_categories_name UNIQUE (categories_name)
) ;
EOD;
      $CLICSHOPPING_Db->exec($sql);
    }
