...
namespace ClicShopping\Apps\Tools\AdministratorMenu\Sql\Postgres;
...
class Postgres
{
  ...
  private static function installDb(): void
  {
    ...
    $Qcheck = $CLICSHOPPING_Db->query("SELECT to_regclass(':table_administrator_menu')");

    if ($Qcheck->fetch() === false) {
      $sql = <<<EOD
    CREATE TABLE :table_administrator_menu (
      administrator_menu_id serial PRIMARY KEY,
      administrator_menu_variable varchar(250) NOT NULL,
      customers_group_id int DEFAULT 0 NOT NULL,
      administrator_menu_type smallint DEFAULT 0 NOT NULL
    ) CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

    ALTER SEQUENCE :table_administrator_menu_administrator_menu_id_seq RESTART WITH 1;
EOD;
      $CLICSHOPPING_Db->exec($sql);
    }

    $Qcheck = $CLICSHOPPING_Db->query("SELECT to_regclass(':table_administrator_menu_description')");
    ...
    CREATE TABLE :table_administrator_menu_description (
      ...
      administrator_menu_description text,
