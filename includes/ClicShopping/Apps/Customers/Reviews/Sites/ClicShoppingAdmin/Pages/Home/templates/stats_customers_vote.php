                  ...
                  <div class="card-text">
                    <div class="col-sm-12">
                      <span class="float-end">
                        ...
                        <div
                          class="col-sm-12 text-white"><?php echo $CLICSHOPPING_Reviews->getDef('text_average_no') . ' ' . $count_vote_no . ' - ' . round($percentage_no, 2) . '%'; ?></div>
                      </span>
                    </div>
                  </div>
                </div>
  ...
  <table
    ...
    <?php
      $QReport = $CLICSHOPPING_Reviews->db->prepare('select distinct SQL_CALC_FOUND_ROWS  r.id,
                                                                                         r.products_id,
                                                                                         r.reviews_id,
                                                                                         SUM(CASE WHEN r.vote = 1 THEN 1 ELSE 0 END) AS vote_yes,
                                                                                         SUM(CASE WHEN r.vote = 0 THEN 1 ELSE 0 END) AS vote_no,
                                                                                         SUM(CASE WHEN r.vote IN (0, 1) THEN 1 ELSE 0 END) AS total_votes,
                                                                                         r.sentiment,
                                                                                         p.products_image
