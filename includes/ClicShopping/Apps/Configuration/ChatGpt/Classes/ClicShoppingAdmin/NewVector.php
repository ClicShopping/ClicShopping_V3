...
//***********
  ...
  /**
   * Configure and initialize Doctrine ORM settings
   * @return array Array containing connection parameters and configuration
   *               ['connectionParams' => array, 'config' => object]
   */
  private static function Orm()
  {
    // Récupération des paramètres de connexion depuis ClicShopping
    $database = CLICSHOPPING::getConfig('db_database');

    // Configuration de Doctrine (chemin vers vos entités, ici supposé être "src/Entity")
    $isDevMode = true;
    $paths = [__DIR__ . '/src/Entity'];

    // Utilisation de ORMSetup pour la configuration
    $config = ORMSetup::createAttributeMetadataConfiguration($paths, $isDevMode);

    // Préparation des paramètres de connexion pour Doctrine
    $connectionParams = [
      'driver' => 'pdo_sqlite',
      'path'   => $database,
    ];
   ...
   *
   * @return array The configured EntityManager instance.
   */
  private static function EntityManager() : array
  {
    $orm = self::Orm();
    $connectionParams = $orm['connectionParams'];
    $config = $orm['config'];

    // Création de l'EntityManager avec DriverManager
    $connection = DriverManager::getConnection($connectionParams, $config);
    $entityManager = new EntityManager($connection, $config);


    /*
       // Stocker les embeddings dans la base via DoctrineVectorStore
       $vectorStore = new DoctrineVectorStore(static::Orm(), PlaceEntity::class);
