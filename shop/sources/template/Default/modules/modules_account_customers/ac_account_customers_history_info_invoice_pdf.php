...
// Display the pdf type in function the status
        if ($count_history_info > 0) {
          ...
          if ($display_support == 0) {
            $print_invoice_pdf = HTML::link(CLICSHOPPING::link(null, 'Account&OrderInvoice&order_id=' . (int)$_GET['order_id'])  . '" target="_blank" rel="noreferrer"', HTML::Button(null, 'fas fa-file-pdf fa-4x HistoyInfoPDF'));
          }
        }
