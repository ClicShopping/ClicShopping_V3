      ...
      <td>
        <table class="table table-sm table-hover table-striped">
          ...
          <tr class="dataTableHeadingRow">
            ...
            <td class="text-md-right"><?php echo $CLICSHOPPING_Manufacturers->getDef('table_heading_action'); ?>&nbsp;</td>
          </tr>
          </thead>
          <tbody>
<?php
...
?>
                  <input type="checkbox" name="selected[]" value="<?php echo $Qmanufacturers->valueInt('manufacturers_id'); ?>" />
<?php
      }
?>
                </td>
...
<?php
  if ($listingTotalRow > 0) {
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="col-md-6 float-md-left pagenumber hidden-xs TextDisplayNumberOfLink"><?php echo $Qmanufacturers->getPageSetLabel($CLICSHOPPING_Manufacturers->getDef('text_display_number_of_link')); ?></div>
        <div class="float-md-right text-md-right"><?php echo $Qmanufacturers->getPageSetLinks(CLICSHOPPING::getAllGET(array('page', 'info', 'x', 'y'))); ?></div>
      </div>
    </div>
    <?php
  } // end $listingTotalRow
