...
?>
                  <input type="checkbox" name="selected[]" value="<?php echo $Qsuppliers->valueInt('suppliers_id'); ?>" />
        <?php
      }
?>
                </td>
...
<?php
  if ($listingTotalRow > 0) {
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="col-md-6 float-md-left pagenumber hidden-xs TextDisplayNumberOfLink"><?php echo $Qsuppliers->getPageSetLabel($CLICSHOPPING_Suppliers->getDef('text_display_number_of_link')); ?></div>
        <div class="float-md-right text-md-right"><?php echo $Qsuppliers->getPageSetLinks(CLICSHOPPING::getAllGET(array('page', 'info', 'x', 'y'))); ?></div>
      </div>
    </div>
    <?php
  }
