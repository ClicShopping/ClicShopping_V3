  ...
  class Upgrade extends \ClicShopping\OM\Modules\AdminDashboardAbstract
  {
    ...
    public function getOutput()
    {
      ...
      if ($new_version === true) {
        ...
        $output .= '<span class="col-md-8"><strong>' . $this->app->getDef('module_admin_dashboard_clicshopping_update_app_text_warning_upgrade') . ' : ' . $current_version . '  => ' . $core_info->version . ' - ' . $core_info->date . '<br />'. $core_info->description . '  </strong></span>';
        $output .= '<span class="col-md-4 text-md-right"><a href="https://github.com/ClicShopping/ClicShopping_V3/archive/master.zip" target="_blank" rel="noreferrer">' . HTML::button($this->app->getDef('module_admin_dashboard_clicshopping_update_app_button', 'primary')) . '</a></span>';
        $output .= '</div>';
        $output .= '</div>';
