 ...
 */
  ...
  class OrdersStatus extends \ClicShopping\OM\AppAbstract {
    ...
    protected $identifier = 'ClicShopping_OrdersStatus_V1';
        ...
        if ($dir = new \DirectoryIterator($directory)) {
          foreach ($dir as $file) {
            if (!$file->isDot() && $file->isDir() && is_file($file->getPathname() . '/' . $file->getFilename() . '.php')) {
               $class = 'ClicShopping\Apps\Configuration\OrdersStatus\Module\ClicShoppingAdmin\Config\\' . $file->getFilename() . '\\' . $file->getFilename();

              if (is_subclass_of($class, 'ClicShopping\Apps\Configuration\OrdersStatus\Module\ClicShoppingAdmin\Config\ConfigAbstract')) {
                ...
                trigger_error('ClicShopping\Apps\Configuration\OrdersStatus\OrdersStatus::getConfigModules(): ClicShopping\Apps\Configuration\OrdersStatus\Module\ClicShoppingAdmin\Config\\' . $file->getFilename() . '\\' . $file->getFilename() . ' is not a subclass of ClicShopping\Apps\Configuration\OrdersStatus\Module\ClicShoppingAdmin\Config\ConfigAbstract and cannot be loaded.');
              }
            }
          }
