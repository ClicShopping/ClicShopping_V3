...
?>
              <a onclick="$('delete').prop('action', ''); $('form').submit();"
                 class="button"><span><?php echo HTML::button($CLICSHOPPING_PageManager->getDef('button_delete'), null, null, 'danger'); ?></span></a>
            </span>
        </div>
      </div>
    </div>
...
?>
              <th
                class="text-md-center"><?php echo $CLICSHOPPING_PageManager->getDef('table_heading_customers_group'); ?></th>
<?php
  }
?>
...
?>
            <td class="dataTableContent"
                width="1.25rem;"><?php echo '<a href="' . HTTP::getShopUrlDomain() . 'index.php?Info&Content&' . 'pages_id=' . $Qpages->valueInt('pages_id') . '" target="_blank" rel="noreferrer">' . HTML::image($CLICSHOPPING_Template->getImageDirectory() . 'icons/preview_catalog.png', $CLICSHOPPING_PageManager->getDef('icon_preview')) . '</a>'; ?></td>
<?php
      } else {
?>
            <td></td>
